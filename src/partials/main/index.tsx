...
const MainPage = () => {
    ...
    return (
        ...
        {view === View.INIT ? <NameEntry onClick={onCalculateClick} /> : <ChampSelect summoners={summoners} champs={champs} onBack={() => setView(View.INIT)} />}
        </div>
    ...
    return(
        <div style={{ width: '700px', border: '1px white solid', borderRadius: '5px'}}>
            ...
            <div className="disp-flex train__justify-center" style={{ padding: 20 }}>
                <textarea value={text} onChange={e => setText(e?.target?.value?.replace(/\n\n/g, '\n'))} rows={10} style={{ width: '300px', fontSize:'16px' }} 
...
const Summoner = ({ summoner }) => {
    ...
    return (
        <div style={{ border: '1px white solid', padding: 10, marginTop: 10, width: 200,  }}>
            {summoner}
        </div>
    )
}
